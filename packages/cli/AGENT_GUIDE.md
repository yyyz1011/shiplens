...
`shiplens_get_run` retrieves current statuses and append-only assessment history. The `review_website` MCP prompt also describes this loop. The same six operations are methods of `ReviewWorkspace` from `shiplens/review`; `getRun` takes a string run ID.
