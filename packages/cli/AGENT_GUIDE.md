...
## Approved acceptance locks

A host may configure `acceptanceLock: {file, sha256}` (CLI/MCP) or `{lock, sha256}` (API). Call `shiplens_check_plan_lock({data})` before verification to inspect differences. `shiplens_verify` enforces this check itself and returns structured changes on `SHIPLENS_PLAN_LOCK_BLOCKED`. Do not weaken checks, delete criteria, narrow viewports, edit the approved lock, or replace its expected digest merely to make a task pass. Restore the reviewed standard or ask the maintainer to review an intentional change. There is no MCP lock-update tool. Locked hosts require `verify`; `collect`, `recheck` and `verifyDelivery` cannot bypass this guard.

The lock only compares the approved definition and policy. It does not prove a requirement true, judge whether a change is an improvement, or protect against an actor controlling the host and expected digest. Runtime input values and authentication are not pinned. Keep manual assessments evidence-based even after a matching lock check.
