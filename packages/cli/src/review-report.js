...
export function acceptanceGate(run, { failOn = 'error' } = {}) {
  ...
  return {
    ...
    failOn,
    reasons,
    note: 'Gate applies to recorded caller assessments and observed scope; it does not verify model reasoning.',
  };
}
export function acceptanceMarkdown(run, gate) {
  return `# ShipLens acceptance review\n\nRun: ${line(run.runId)}\n\nGate: **${gate.passed ? 'PASS' : 'BLOCKED'}**\n\n${line(gate.note)}\n\n${gate.reasons.map((x) => '- ' + x).join('\n')}\n\n| Requirement | Status | Judgment | Evidence |\n| --- | --- | --- | --- |\n${run.requirements.map((r) => `| ${line(r.id)}: ${line(r.description)} | ${r.status} | ${line(r.assessment?.note || 'Awaiting assessment')} | ${line(r.assessment?.evidenceIds.join(', ') || '')} |`).join('\n')}\n\n## Machine checks\n\n${Object.entries(
    run.machine.summary,
  )
    .map(([k, v]) => `- ${k}: ${v}`)
    .join(
