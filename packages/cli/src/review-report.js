...
</style></head><body><header>ShipLens</header><main><h1>${t('Acceptance review', '验收报告')}</h1><p class="muted">${escape(run.createdAt)} · <code>${escape(run.runId)}</code></p>${run.planSource ? `<p>${escape(run.planSource.name)}</p><details><summary>${t('Plan fingerprint', '验收文件指纹')}</summary><code>${escape(run.planSource.sha256)}</code></details>` : ''}<div class="gate"><strong>${gate.passed ? t('Recorded checks passed', '记录内的检查已通过') : t('Review requires attention', '验收仍需处理')}</strong><p>${t('AI judgments and machine checks are listed separately. Pending work and missing evidence remain visible.', 'AI 判断与机器检查分别展示，未完成的判断与缺失证据保留在报告中。')}</p>${gate.reasons.length ? `<p>${escape(gate.reasons.join(' · '))}</p>` : ''}</div>${gate.planLock ? `<h2>${t('Approved acceptance standard', '已确认的验收标准')}</h2><p>${gate.planLock.matched ? t('The recorded run matches the host-pinned standard and policy.', '本轮记录与主机固定的标准和策略匹配。') : t('The recorded run does not match the host-pinned standard or policy.', '本轮记录与主机固定的标准或策略不匹配。')}</p><details><summary>${t('Approved lock fingerprint', '已确认锁定文件的指纹')}</summary><code>${escape(gate.planLock.sha256)}</code></details>` : ''}${comparisons}<h2>${t('Requirements', '验收项')}</h2><div class="scroll"><table><thead><tr><th>${t('Requirement', '需求')}</th><th>${t('Status', '状态')}</th><th>${t('Judgment', '判断依据')}</th></tr></thead><tbody>${run.requirements.map((r) => `<tr><td>${escape(r.description)}</td><td>${status(r.status)}</td><td>${escape(r.assessment?.note || (r.verification ? t('Configured text checks: ', '配置的文本断言：') + status(r.verification.status) : t('Awaiting assessment', '等待判断')))}</td></tr>`).join('')}</tbody></table></div>${run.requirements
    .map(
      (r) =>
