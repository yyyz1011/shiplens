...
export function mcpDocs() {
  return {
    ...
    description: t(
      'Twenty-four project-scoped tools for your existing image-capable AI client.',
      '二十四个限定项目范围的工具，供现有支持图片的 AI 客户端调用。',
    ),
    body:
      ...
      p(
        'Use <a href="#/docs/check-audit">shiplens_audit_checks</a> to challenge passing text rules with offline counterexamples before trusting a plan.',
        '信任验收文件前，可用 <a href="#/docs/check-audit">shiplens_audit_checks</a> 通过离线反例检查已经通过的文本规则。',
      ) +
      p(
        'For authorized form writes, <a href="#/docs/delivery-proof">shiplens_verify_delivery</a> checks the correlated backend record and injected failure path; get_delivery/read_delivery_evidence retrieve its saved proof.',
        '对已授权的表单写入，<a href="#/docs/delivery-proof">shiplens_verify_delivery</a> 检查关联的后台记录及注入失败分支，get_delivery/read_delivery_evidence 读取保存的结果与证据。',
      ) +
