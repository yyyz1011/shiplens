    ...
    .map(
      (group) =>
        `<section class="reading-nav-group"><h2>${group}</h2>${docs
          ...
          .map(
            (d) =>
              `<a href="${docLink(d.id)}" ${d.id === id ? 'class="active" aria-current="page"' : ''}>${d.id === 'introduction' ? t('Capabilities & comparison', '能力与对比') : d.title}</a>`,
          )
          .join('')}</section>`,
    )
    .join(
...
function docPage(id) {
  ...
  return `${header()}<div class="reading-subnav"><button class="docs-menu-trigger" aria-haspopup="dialog" aria-controls="docs-drawer">${icon('menu')}${t('Menu', '目录')}</button><span>${id === 'introduction' ? t('Capabilities & comparison', '能力与对比') : doc.title}</span><details class="mobile-outline"><summary>${t('On this page', '本页内容')}</summary><nav>${tocLinks(toc, id)}</nav></details></div><aside class="reading-sidebar">${navigation(id)}<div class="reading-version">v${version}<span>MIT</span></div></aside><dialog id="docs-drawer" class="reading-drawer" aria-label="${t('Documentation navigation', '文档目录')}"><div class="drawer-heading"><strong>${t('Documentation', '文档')}</strong><button data-close-drawer aria-label="${t('Close menu', '关闭目录')}">${icon('close')}</button></div>${navigation(id)}</dialog><div class="reading-layout"><main id="main" class="reading-article${id === 'introduction' ? ' reading-home' : ''}"><h1>${doc.title}</h1><p class="reading-description">${doc.description}</p><div class="doc-body">${body}</div><footer class="reading-article-footer"><a href="https://github.com/yyyz1011/shiplens/blob/master/apps/docs/src/${doc.source || 'content.js'}" target="_blank" rel="noopener">${t('View source on GitHub', '在 GitHub 查看源码')} ↗</a><span>ShipLens v${version} · MIT</span></footer><nav class="reading-pagination" aria-label="${t('Documentation pagination', '文档翻页')}">${current > 0 ? `<a href="${docLink(docs[current - 1].id)}"><span>${t('Previous page', '上一篇')}</span><strong>${current === 1 ? t('Capabilities & comparison', '能力与对比') : docs[current - 1].title}</strong></a>` : '<div></div>'}${current < docs.length - 1 ? `<a href="${docLink(docs[current + 1].id)}"><span>${t('Next page', '下一篇')}</span><strong>${docs[current + 1].title}</strong></a>` : ''}</nav></main><aside class="reading-outline"><nav aria-label="${t('On this page', '本页内容')}"><strong>${t('On this page', '本页内容')}</strong>${tocLinks(toc, id)}</nav><a class="outline-support" href="https://github.com/yyyz1011/shiplens/issues" target="_blank" rel="noopener">${t('Found an issue?', '发现问题？')} ↗</a></aside></div>`;
}
const sampleIssues = [
  {
    severity: 'error',
